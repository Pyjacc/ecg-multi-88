...
def predict_with_resnet_lgb():
    ...
    lgb_model_path = args.lgb_others_2d_path
    ...
    logger.info("load resnet model：%s", model_2d_path)
    model = torch.load(model_2d_path, map_location=device)        # todo: 根据不同的模型选择模型路径
    logger.info("load resnet network")
    num_class = len(args.ecg_classes_others)       # todo: 根据不同的模型选择
    num_leads = args.num_leads
    network = resnet342d7_15(input_channels=num_leads, num_classes=num_class).to(device)
