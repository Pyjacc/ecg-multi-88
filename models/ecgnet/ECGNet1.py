...
class ECGNet(nn.Module):
    def __init__(self, input_channel=1, num_classes=12):  # , layers=[2, 2, 2, 2, 2, 2]
        ...
        self.relu = nn.ReLU(inplace=True)
        self.conv1 = nn.Conv2d(input_channel, 32, kernel_size=(1, 50), stride=(1, 2), padding=(0, 0), bias=False)
        self.bn1 = nn.BatchNorm2d(32)
        self.conv2 = nn.Conv2d(32, 32, kernel_size=(1, 16), stride=(1, 2), padding=(0, 0), bias=False)
        ...
        for i, size in enumerate(sizes):
            ...
            self.layers2.add_module('layer{}_2_1'.format(size),
                                    self._make_layer1d(BasicBlock1d, 32 * 12, layers[i][2], stride=2, size=sizes[i][2]))
            self.layers2.add_module('layer{}_2_2'.format(size),
                                    self._make_layer1d(BasicBlock1d, 32 * 12, layers[i][3], stride=2, size=sizes[i][3]))
            self.layers2.add_module('layer{}_2_3'.format(size),
                                    self._make_layer1d(BasicBlock1d, 32 * 12, layers[i][4], stride=2, size=sizes[i][4]))
            self.layers2.add_module('layer{}_2_4'.format(size),
                                    self._make_layer1d(BasicBlock1d, 32 * 12, layers[i][5], stride=2, size=sizes[i][5]))

            self.layers1_list.append(self.layers1)
            self.layers2_list.append(self.layers2)

        # self.drop = nn.Dropout(p=0.2)
        self.fc = nn.Linear(32 * 12 * len(sizes), num_classes)
    ...
    def forward(self, x0):
        ...
        return out, 0.0000001


def ecgnet2d(**kwargs):
    logger.info("using resnet34 1d model", kwargs)
    model = ECGNet()
    return model