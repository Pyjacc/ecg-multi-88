...
class ECGDataset(Dataset):
    def __init__(self, phase, data_dir, label_csv, random):
        ...
        self.classes = args.ecg_classes_normal          # todo:需要根据训练的模型不同而进行调整
    ...
    def __getitem__(self, index: int):
        ...
        # result = np.zeros((self.n_leads, 5000))                              # 10 s, 500 Hz
        # result[:, :] = ecg_data
        ...
        return torch.from_numpy(ecg_data).float(), torch.from_numpy(labels).float(), patient_id
