...
def get_matrix():
    # 1. 读取预测的结果和真实的结果
    answer_path = args.answer_lgb_others       # todo: 根据想要测试的结果文件传入不同的文件路径

    true_labels = []
    predict_labels = []

    true_df = pd.read_csv(args.test_label_others_csv, sep="\t")     # todo: 根据想要测试的结果文件传入不同的文件路径
    ...
    # 2. 将label转换为(4000, 1)的矩阵
    length = len(true_labels)
    true_matrix = np.zeros((length, len(args.ecg_classes_others)))      # (4000, 1) todo
    pred_matrix = np.zeros((length, len(args.ecg_classes_others)))      # (4000, 1) todo

    for row in range(len(true_labels)):
        label = true_labels[row].split(",")
        for col in range(1, 2):     # todo
...
def cal_2_diseases_avg_f1():
    ...
    for i in range(len(args.ecg_classes_others)):       # todo
...
def cal_absolute_correct():
    ...
    for row in range(len(true_matrix)):
        for col in range(len(args.ecg_classes_others)):     # todo
...
if __name__ == "__main__":
    avg_p, avg_r, avg_f1 = cal_2_diseases_avg_f1()        # 类别平均分数
