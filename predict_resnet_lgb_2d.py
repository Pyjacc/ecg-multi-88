...
'''先使用resnet34提取特征，然后将特征送入lightgbm进行多分类预测结果'''
...
from models.resnet.resnet34_rdrop import resnet342d     # todo

tools.seed_torch(args.seed)

def perdict_with_lgb(device, network, model, data_loader, thresholds, answer_path):
    output_list = []
...
def predict_with_resnet_lgb():
    answer_path = args.answer_lgb_2d            # todo
    resnet_model_path = args.model_path_2d
    lgb_model_path = args.lgb_model_path_2d
    threshold_file_path = args.threshold_file_lgb_2d
    ...
    network = resnet342d(input_channels=num_leads, num_classes=num_class).to(device)
    ...
    perdict_with_lgb(device, network, lgb_model, test_loader, thresholds, answer_path)
