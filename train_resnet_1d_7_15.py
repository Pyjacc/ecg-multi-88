...
'''训练resnet模型，使用1维的卷积核，卷积核的大小为7*7和15*15'''

import os
import torch
from torch.utils.data import DataLoader
from models.resnet.resnet34_rdrop import resnet341d7_15     # todo
...
def train_model():
    ...
    network = resnet341d7_15(input_channels=num_leads, num_classes=num_class).to(device)  # todo: 选择不同的模型
    ...
    args.best_f1 = 0
    args.best_epoch = 0
    model_type = 'resnet_1d_7_15'        # todo
...
def train_resnet_1d_7_15_model():
    if args.platform == 'local':
        split_train_dev_test_data()            # split test data and train data
        get_train_dev_test_label_local()       # get train and test labels from trainreference_new.csv
    else:
