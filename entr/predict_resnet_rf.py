...
'''先使用resnet34提取特征，然后将特征送入rf进行多分类预测结果'''
...
def perdict_with_lr(device, network, model, data_loader, thresholds):
    ...
    df_answer.to_csv(args.answer_rf, index=None)


def predict_with_resnet_rf():
    ...
    lgb_model = joblib.load(args.rf_model_path)
    logger.info('predict on test data, please wait...')

    # get thresholds
    thresholds = []
    if os.path.exists(args.threshold_file_rf):
        with open(args.threshold_file_rf, "r", encoding="utf-8") as fr:
            lines = fr.readlines()
            for line in lines:
