...
def get_threshold(model, data, label):
    ...
    with open(args.threshold_file_mlp, "w", encoding="utf-8") as fw:
...
def train_mlp_model():
    ...
    logger.info("train mlp model start, please wait...")
    model = MLPClassifier(hidden_layer_sizes=(100, 50), max_iter=500, seed=args.seed)
    model = OneVsRestClassifier(model)
    model.fit(train_data, train_lable)
    logger.info("train mlp model end, save model")
    joblib.dump(model, args.mlp_model_path)
    logger.info("save mlp model success")

    # get optimal thresholds
    model = joblib.load(args.mlp_model_path)            # 使用已经训练好的lr模型寻找阈值
    get_threshold(model, dev_data, dev_label)
    logger.info("train mlp model success!")
