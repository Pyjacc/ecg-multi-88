...
'''先使用resnet34提取特征，然后将特征送入xgboost进行多分类预测结果'''
...
def perdict_with_xgb(device, network, model, data_loader, thresholds):
    output_list = []
    ...
    df_answer.to_csv(args.answer_xgb, index=None)       # todo：根据使用的模型不同，将answer写入不同的文件中


def predict_with_resnet_xgb():
    ...
    lgb_model = joblib.load(args.xgb_model_path)        # todo：根据使用的模型不同，选择相应的模型路径
    logger.info('predict on test data, please wait...')

    # get thresholds
    thresholds = []
    if os.path.exists(args.threshold_file_xgb):         # todo：根据使用的模型不同，选择相应的阈值路径
        with open(args.threshold_file_xgb, "r", encoding="utf-8") as fr:
            lines = fr.readlines()
            for line in lines:
    ...
    perdict_with_xgb(device, network, lgb_model, test_loader, thresholds)
