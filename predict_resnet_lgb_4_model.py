...
'''先使用resnet34的4个模型提取特征，然后将特征送入lightgbm进行多分类预测结果'''
...
def perdict_with_lgb(device, network1d1, network1d2, network2d1, network2d2, lgb_model,
                     data_loader, thresholds, test_label_csv, answer_path):
    output_list = []

    for i, (data, label) in enumerate(tqdm(data_loader)):
        data = data.to(device)
        output1 = network1d1(data)
        output2 = network2d1(data)
        output3 = network1d2(data)
        output4 = network2d2(data)
...
def predict_with_resnet_lgb():
    answer_path = args.answer_lgb_4_model            # todo
    ...
    model1d1 = torch.load(model_1d_path_7_15, map_location=device)        # todo
    model1d2 = torch.load(model_1d_path_3_5, map_location=device)
    model2d1 = torch.load(model_2d_path_7_15, map_location=device)
    model2d2 = torch.load(model_2d_path_3_5, map_location=device)

    num_class = len(args.ecg_classes)
    num_leads = args.num_leads
    network1d1 = resnet341d7_15(input_channels=num_leads, num_classes=num_class).to(device)
    network1d2 = resnet341d3_5(input_channels=num_leads, num_classes=num_class).to(device)
    network2d1 = resnet342d7_15(input_channels=num_leads, num_classes=num_class).to(device)
    network2d2 = resnet342d3_5(input_channels=num_leads, num_classes=num_class).to(device)
    ...
    perdict_with_lgb(device, network1d1, network1d2, network2d1, network2d2, lgb_model,
