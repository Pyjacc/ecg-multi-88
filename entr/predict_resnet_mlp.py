...
'''先使用resnet34提取特征，然后将特征送入mlp进行多分类预测结果'''
...
def perdict_with_mlp(device, network, model, data_loader, thresholds):
    output_list = []
    ...
    df_answer.to_csv(args.answer_mlp, index=None)       # todo: 需要根据使用的模型不同将answer写入到不同的文件中。


def predict_with_resnet_mlp():
    ...
    lgb_model = joblib.load(args.mlp_model_path)        # todo: 需要根据使用的模型不同加载不同的模型。
    logger.info('predict on test data, please wait...')

    # get thresholds
    thresholds = []
    if os.path.exists(args.threshold_file_mlp):         # todo: 需要根据使用的模型不同加载不同的阈值文件。
        with open(args.threshold_file_mlp, "r", encoding="utf-8") as fr:
            lines = fr.readlines()
            for line in lines:
    ...
    perdict_with_mlp(device, network, lgb_model, test_loader, thresholds)
