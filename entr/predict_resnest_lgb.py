...
'''先使用resnest50提取特征，然后将特征送入lightgbm进行多分类预测结果'''
...
from models.resnest.resnest_feature_2048 import resnest50

tools.seed_torch(args.seed)

def perdict_with_lgb(device, network, model, data_loader, thresholds):
    output_list = []
    ...
    df_answer.to_csv(args.answer_lgb, index=None)       # todo：根据使用的模型不同，将answer写入不同的文件中
...
def predict_with_resnet_lgb():
    ...
    network = resnest50(input_channels=num_leads, num_classes=num_class).to(device)
    ...
    lgb_model = joblib.load(args.lgb_model_path)        # todo：根据使用的模型不同，选择相应的模型路径
    logger.info('predict on test data, please wait...')

    # get thresholds
    thresholds = []
    if os.path.exists(args.threshold_file_lgb):     # todo：根据使用的模型不同，选择相应的阈值路径
        with open(args.threshold_file_lgb, "r", encoding="utf-8") as fr:
            lines = fr.readlines()
            for line in lines:
    ...
    perdict_with_lgb(device, network, lgb_model, test_loader, thresholds)
