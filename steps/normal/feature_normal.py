...
from steps.normal.dataset_normal import ECGDataset        # todo: 根据不同的模型选择不同的ECGDataset
...
class GetFeature:
    ...
    def join_train_dev_labels_csv(self):
        result_train = []
        df_train = pd.read_csv(args.train_label_normal_csv, sep='\t')
        ...
        result_dev = []
        df_dev = pd.read_csv(args.dev_label_normal_csv, sep='\t')
        ...
        df = pd.DataFrame(data=result, columns=['patient_id'] + args.ecg_classes_normal)
        df['random'] = np.random.RandomState(args.seed).permutation(random_num)
        columns = df.columns
        df[columns].to_csv(args.train_dev_label_normal_csv, index=None)
    ...
    def get_feature_from_resnet(self, model_1d_path, renset1d, feature_path):
        ...
        train_dataset = ECGDataset('train', data_dir, args.train_dev_label_normal_csv, train_random_num)    # todo: train_label_csv
        ...
        num_class = len(args.ecg_classes_normal)       # todo： 根据模型选择相应的ecg_classes
    ...
    def get_feature_from_2_resnet(self, model_1d_path, model_2d_path, renset1d, renset2d, feature_path):
        ...
        logger.info('load train data')
        train_random_num = tools.get_random_num(seed=args.seed)
        train_dataset = ECGDataset('train', data_dir, args.train_dev_label_normal_csv, train_random_num)       # todo
        train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=False,
                                  num_workers=args.num_workers, pin_memory=True)
